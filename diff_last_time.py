...
def ReadFile(fichier_html_graphs, file_path):
    ...
    if min_gap < 0:
        sprintf("min gap is a broken value {0}, fix this".format(min_gap))
    ...
    sprintf("最小间隔值: {0}".format(min_gap))
    ...
    sprintf("平均帧率: {0}".format(len(y_values) * 1000 / (max(x_values) - min(x_values))))
